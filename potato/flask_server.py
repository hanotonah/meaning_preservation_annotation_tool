...
def annotate_page(username=None, action=None):
    ...
    if action == "home":
        ...
        if result_code == "all instances have been assigned":
            return render_template(
                "error.html",
                error_message="Er zijn inmiddels al genoeg antwoorden verzameld voor de survey. Toch bedankt voor uw interesse!",
            )
...
def parse_html_span_annotation(html_span_annotation):
    ...
    paragraph_separator = '<span style="visibility:hidden">//</span>'
    
    while True:    
      soup = BeautifulSoup(s, "html.parser")
      annotation_container = soup.find("span", class_="span_container")
      #this is the string we would find in the original text
      annotation_container_orig_str = str(annotation_container).replace("<br/>","<br>")
      #check if more annotations need to be processed
      if annotation_container == None or soup == None:
        break
      #if so, extract annotation
      annotation_label = annotation_container.div.get_text()
      #but we need to process potential <br>s with ad-hoc rules,
      # or they will be stripped by the soup element when obtaining text
      # and when returning to prev page the \n will be missing and text will be weird
      br_free_container = copy.copy(annotation_container)
      for br in br_free_container.find_all("br"):
        br.replace_with("\n")
      #same treatment for separators
      for span in br_free_container.find_all("span"):
      	span.replace_with("###SPAN###")
      annotation_text = br_free_container.get_text().replace(annotation_label,"")
      annotation_start = s.find(annotation_container_orig_str)
      annotation_end = annotation_start + len(annotation_text.replace("\n","<br>").replace("###SPAN###",paragraph_separator))
      ann = {
            "start": annotation_start,
            "end": annotation_end,
            "span": annotation_text.replace("\n","<br>").replace("###SPAN###",paragraph_separator), #readd any <br> so that annotation is re-loaded correctly
            "annotation": annotation_label,
        }
      if (annotation_start == -1):
      	print("Found a bug! Annotation is not found in string. Will introduce errors in data if users go back rather than losing info")
      	print(f"{ann=}\n\n{annotation_container=}\n\n{annotation_container_orig_str=}\n\n{s=}\n\n{soup=}\n\n\n")
      	annotations.append(ann)
      	s = str(soup)
      	continue

      annotations.append(ann)
      #remove this annotation from the original text to make sure character counting
      #works as expected
      s = s.replace(annotation_container_orig_str, ann["span"])
